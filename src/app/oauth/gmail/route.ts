...
export async function GET(req: Request) {
  ...
  try {
    ...
    const existingAccount = await db
      ...
      .where(
        and(
          ...
          eq(connectedAccounts.serviceType, 'gmail')
        )
      )
      .limit(1);
    ...
    if (existingAccount.length > 0) {
      ...
      const mergedScopes = [...new Set([...existingScopes, ...gmailScopes])];
      ...
      await db.insert(connectedAccounts).values({
        ...
        tokenExpiresAt: tokens.expiry_date ? new Date(tokens.expiry_date) : null,
        scopesGranted: gmailScopes,
        isActive: true,
      });
    }

    return NextResponse.redirect(new URL('/accounts?success=gmail_connected', url.origin));
  } catch (err: any) {
